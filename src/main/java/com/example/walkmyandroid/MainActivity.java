...
public class MainActivity extends AppCompatActivity implements FetchAddressTask.OnTaskCompleted {
    ...
    private void startTrackingLocation() {
        ...
        if (ActivityCompat.checkSelfPermission(this, Manifest.permission.ACCESS_FINE_LOCATION) != PackageManager.PERMISSION_GRANTED) {
            ...
            mFusedLocationProviderClient.getLastLocation().addOnSuccessListener(new OnSuccessListener<Location>() {
                ...
                public void onSuccess(Location location) {
                    // if location is available, set latitude, longitude, and time values in text view
                    if (location != null) {
        ...
        mLocationTextView.setText(getString(R.string.address_text, getString(R.string.loading), System.currentTimeMillis(), distance));
    ...
    public void onTaskCompleted(String result) {
        if (mTrackingLocation) {
            // if both location objects are not null, then calculate distanceTo
            if (lastLocation != null && currentLocation != null){
                distance = lastLocation.distanceTo(currentLocation);
                // convert meters to kilometers
                distance = distance / 1000;
                mLocationTextView.setText(getString(R.string.address_text, result, System.currentTimeMillis(), distance));
            }
            else{
                mLocationTextView.setText(getString(R.string.address_text, result, System.currentTimeMillis(), distance));
            }
